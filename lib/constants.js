...
export const OUT_REGISTRY_DIR_PATH = path.join(OUT_ROOT_DIR_PATH, 'registry')
...
export const REGEX_STRING = {
  ...
  formattedHtmlMessage: /(?<all>(?<before>[\{\n\s]*.*)(?<main><FormattedHTMLMessage(?<props>(?<defaultMessage>[\n\s]*defaultMessage=\{[\w\.]+\}[\n\s]*)*(?<id>[\n\s]*id=\{[\w\.]+\}[\n\s]*)*(?<spread>[\n\s]*\{[\n\s]*\.\.\..+[\n\s]*\}[\n\s]*)*(?<values>values=\{\{[\n\s]*.+[\n\s]*\}\}[\n\s]*)*)\/\>)(?<after>[\}\n\s]*))/g,
  importedIntl: /(?<all>import\s+(?<name>[\w]+)\s+from\s+'(?<source>.*\.i18n)';)/g,
  reactIntlUsage: /(?<all>import.+{[\s]*(?<named>.+)[\s]*}.+'react-intl';)/g
}
